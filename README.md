...
> See [`PRD.md`](PRD.md) for full product scope and the engineering standard, and
> [`CLAUDE.md`](CLAUDE.md) for architecture conventions when contributing.
...
- **Data:** mock repositories behind interfaces (no real backend yet)
...
features). The data layer is mock-backed; persistence (secure token + prefs)
and a real backend are the next milestones.
